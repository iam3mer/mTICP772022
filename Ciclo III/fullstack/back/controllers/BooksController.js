...
module.exports = class BooksController {
  ...
  static async updateBook (request, response) {
    try {
      ...
      if (book.acknowledged != false) {
        ...
        response.status(404).json({message: `Error al actualizar: no existe un documento con isbn ${isbn}`})
      }
    } catch (error) {
      response.status(400).json({message: error.message});
    }
  ...
  static async deleteBook (request, response) {
    try {
      const isbn = request.params.isbn;
      const book = await booksModel.deleteOne({isbn: isbn});
      if (book.deletedCount == 0) {
        response.status(200).json();
      } else {
        response.status(200).json({message: `Error al eliminar: no existe un documento con isbn ${isbn}`})
      }
      response.status(200).json(book);
    } catch (error) {
      response.status(400).json({message: error.message});
